...
# BLEU-4 score - parameters setting
...
# compute the BLEU-4 score of test data
...
def Plot(mode, data):
    ...
    if mode == "loss" : info = ["Training Loss", "Epochs", "CrossEntropy Loss"]
    elif mode == "score" : info = ["BLEU-4 Score (training state)", "Epochs", "BLEU-4 Score"]
...
def blue_score(mode):
	...
	elif(mode == 'New Test') : data = New_Test_Data

	for idx in data.index:
	    input = data.loc[idx, 'input'][0]
	    ...
	    bleu_score_.append(compute_bleu(target, pred))

	print('='*30)
	print(mode, "Data BLEU-4 Score: ", round(sum(bleu_score_)/len(bleu_score_),4) )
	print("----------")
